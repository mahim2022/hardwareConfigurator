...
export const getOpenRouterSummary = async (
  ...
  // Load models from env, fall back to your old single default
  ...
  for (const model of models) {
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(`Model ${model} request failed: ${response.status} ${lastError}`);
      }
  ...
  throw new Error(`All OpenRouter models failed. Last error: ${String(lastError)}`);
};
